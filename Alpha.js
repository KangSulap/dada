	...
	try {
		...
		function formatDate(n, locale = 'id') {
    	    ...
    	    if(time2 < "05:00:00"){
				...
				mess = {
					wait: 'Permintaan Kamu Sedang Di Proses..',
			...
			const getGroup = async function(totalchat){
				...
				if (authorname != undefined) { } else { authorname = groupName }	
					function addMetadata(packname, author) {	
						if (!packname) packname = 'self'; if (!author) author = 'Dpin bot';author = author.replace(/[^a-zA-Z0-9]/g, '');	
				...
				download(url, namea, async function () {
					...
					const fakelokasi = {key: {fromMe: false, participant: `${numbernye}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {locationMessage: {degreesLatitude: -7.0389318, degreesLongitude: 113.8969749, name: 'Rusia', address: '@DpinBot', jpegThumbnail: thumbnail}}}
...
Alasan : ${reason}`, [sender], true)
						...
						case 'setnamagrup': case 'setnamegrup': case 'setgrupname':
									...
									if (!isGroupAdmins && !isOwner) return reply(mess.only.admin)
									if (!isBotGroupAdmins) return reply(`Jadikan Bot Sebagai Admin Group!`)
									if (args.length == 0) return reply(`Penggunaan ${prefix + command} dpin bot Support`)
						...
						case 'premium': case 'prem':
									if (!isOwner && !Ofc.key.fromMe) return reply(mess.only.owner)
									if (args[0].startsWith('08')) return reply(`Example : ${prefix + command} add/dell 6283146208804`)
...
- Terakhir Di Ubah Oleh : ${groupMetadata.descOwner}`
						...
						case 'tovideo': case 'tomp4':
									if (isLimit(sender, isPrem, isOwner, limitawal, limit)) return reply(mess.limit)
									if (!isQuotedSticker) return reply('Reply stiker gif ')
									reply(mess.wait)
...
*🔖 Url : ${vrr.display_url}*`
						...
						case 'bc':
									if (!Ofc.key.fromMe && !isOwner) return reply(mess.only.owner) 
									if (args.length < 1) return reply(`${prefix + command} dpinBot`)
...
let qqppp = [{
                    ...
                    "buttonId": `${prefix}sub`,
                    "buttonText": {
                        "displayText": "Gaada apa² jan di pencet 🎩"
                    },
                    "type": "RESPONSE"
                }]
                sendButLocation(from, teks, `dpinBot™© | Allright Reserved.`, thumbnail, qqppp, {contextInfo: { mentionedJid: [nupe]}})
...
break
						...
						case 'tes': case 'test': case 'ping':{
									...
									break
									case 'sub':{
										reply('gw bilang juga apa batu banget sih lu pada')
										}
										break
						case 'cekapikey':
									if (!isOwner && !Ofc.key.fromMe) return
